    ...
    <div className="min-h-full bg-gradient-to-b from-slate-50 via-white to-white p-6 md:p-10">
      <div className="mx-auto flex max-w-5xl flex-col gap-8">
          ...
          <div className="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-white/80">
                {t("actionPages.archive.badge")}
              </p>
              <h1 className="mt-2 text-3xl font-semibold leading-tight md:text-4xl">
                {t("actionPages.archive.title")}
              </h1>
              <p className="mt-3 max-w-2xl text-base text-slate-100/90">
                {t("actionPages.archive.subtitle")}
              </p>
            </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Block
            icon={FiFileText}
            title={t("actionPages.archive.cards.justifications.title")}
            desc={t("actionPages.archive.cards.justifications.desc")}
            onClick={() => navigate("/dashboard/employee-affairs/archive/justifications")}
          />
        </div>
      </div>
    </div>
