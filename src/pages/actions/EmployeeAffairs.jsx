...
import { FiShield, FiHeart, FiClock, FiArchive } from "react-icons/fi";

const InfoCard = ({ icon: Icon, title, desc, onClick }) => (
  <button
    type="button"
    onClick={onClick}
...
export default function EmployeeAffairs() {
  ...
  const cards = [
    {
      key: "socialInsurance",
      icon: FiShield,
      path: "/dashboard/employee-affairs/social-insurance",
    },
    {
      key: "healthInsurance",
      icon: FiHeart,
      path: "/dashboard/employee-affairs/health-insurance",
    },
    {
      key: "attendance",
      icon: FiClock,
      path: "/dashboard/employee-affairs/attendance",
    },
    {
      key: "archive",
      icon: FiArchive,
      path: "/dashboard/employee-affairs/archive",
    },
  ];
    ...
    <div className="min-h-full bg-gradient-to-b from-slate-50 via-white to-white p-6 md:p-10">
      <div className="mx-auto flex max-w-5xl flex-col gap-8">
          ...
          <div className="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-white/80">
                {t("actionPages.employeeAffairs.badge")}
              </p>
              <h1 className="mt-2 text-3xl font-semibold leading-tight md:text-4xl">
                {t("actionPages.employeeAffairs.title")}
              </h1>
              <p className="mt-3 max-w-2xl text-base text-indigo-50/90">
                {t("actionPages.employeeAffairs.subtitle")}
              </p>
            </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {cards.map(({ key, icon, path }) => (
            <InfoCard
              key={key}
              icon={icon}
              title={t(`actionPages.employeeAffairs.cards.${key}.title`)}
              desc={t(`actionPages.employeeAffairs.cards.${key}.desc`)}
